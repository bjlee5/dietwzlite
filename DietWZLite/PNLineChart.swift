...
//                        let pointContainsPath:Bool = CGPathContainsPoint((path as! UIBezierPath).cgPath, nil, p1, false)
                        ...
                        let pointContainsPath:Bool = (path as! UIBezierPath).cgPath.contains(p1)
    ...
    */
    ...
    func heightOfString(_ text: NSString!, width: CGFloat, font: UIFont) -> CGFloat{
        var ch:CGFloat!
        var size:CGSize = CGSize(width: width, height: CGFloat.greatestFiniteMagnitude)

        let tdic:NSDictionary = NSDictionary(objects: [font, NSFontAttributeName], forKeys: [])
        size = text.boundingRect(with: size, options: NSStringDrawingOptions.usesFontLeading , attributes: tdic as? [String : AnyObject], context: nil).size
        ch = size.height
        return ch
